...
public class MultiLayerPerceptron {
    ...
    public double[] feedForward(double[] input) {
        ...
        for (int i = 0; i < fLayers[fLayers.length-1].Size; i++)
            output[i] = fLayers[fLayers.length].Neurons[i].Value;
        return output;
    ...
    public double backPropogate(double[] input, double[] output) {
    ...
    public void train(double[][] X_train, double[][] y_train, int epochs) {
        for (int epoch = 0; epoch < epochs; epoch++) {
            ...
            for (int i = 0; i < X_train.length; i++)
                loss += backPropogate(X_train[i], y_train[i]);
            loss /= X_train.length;
    ...
    private double accuracy(double[][] x_train, double[][] y_train) {
        return -1;
    }
}
